...
无头复验（用 kanban 的 .venv，内含 playwright+chromium）：
```bash
...
## 附：kanban 仓零改动核验
本 spike 全程只读复制 kanban 源码到 `vendor-ref/` 与 `static/markdown.js`，原仓一字节未动。收尾 `git -C /Users/a1234/Documents/AI-Agent-Hub/kanban-personal status --short` 输出为空（见执行报告）。
